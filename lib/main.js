...
async function run_block_server() {
  ...
  http.createServer(function(req, res) {
    this.close();
    logger.info('killing blocking server');
    res.end('killing blocking server\n');
    registerNode(args.nodeconfig, args.address, args.port);
  }).listen(port, host);
}
...
async function main (args = null) {
  ...
  try {
    ...
    if (args.nodeconfig !== null) {
        if (args.blockRegister) {
          await run_block_server();
        } else {
